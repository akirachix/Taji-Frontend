    ...
    <nav className="bg-white z-50 md:top-0">
      <div className="flex items-center justify-between h-20">
        ...
        <div className="hidden md:flex space-x-24 font-bold text-black mr-[5%] text-[20px] mt-[4%] md:mb-[3%] darker-grotesque">
          <Link href="/pwa/Homepage" className="text-red-500">Home</Link>
          <Link href="/pwa/uploadimage" className="text-black">Check Drug Status</Link>
          <Link href="/pwa/map" className="text-black">Pharmacy Finder</Link>
        </div>
      </div>
      ...
      {isOpen && (
        ...
        <button onClick={() => setIsOpen(false)} className="absolute top-5 right-5 text-black focus:outline-none">
          ...
          <Link href="/pwa/Homepage" className="block text-red-500">Home</Link>
          <Link href="/pwa/uploadimage" className="block text-black">Check Drug Status</Link>
          <Link href="/pwa/map" className="block text-black">Pharmacy Finder</Link>
        </div>
      )}

      
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white shadow-t-md flex justify-around items-center py-3 z-50">
        <Link href="/pwa/Homepage" className="flex flex-col items-center text-black">
          <FaHome size={24} />
          <span className="text-xs">Home</span>
        </Link>
        <Link href="/pwa/uploadimage" className="flex flex-col items-center text-black">
          <FaInfoCircle size={24} />
          <span className="text-xs">Status</span>
        </Link>
        <Link href="/pwa/map" className="flex flex-col items-center text-black">
